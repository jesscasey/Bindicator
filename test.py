...
class TestBindicator(unittest.TestCase):
    ...
    @patch("builtins.open", new_callable=mock_open, read_data="AB1,foo,CouncilX\n")
    @patch("main.next_bin_day")
    def test_find_council_found(self, mock_next_bin_day, mock_filey):
        postcode = "AB1"
        with patch("builtins.print") as mock_print:
            main.find_council(postcode)
            mock_print.assert_any_call("The local authority for AB1 is CouncilX.")
    ...
    @patch("builtins.open", new_callable=mock_open, read_data="CouncilX,http://example.com\n")
    @patch("main.driver")
    def test_next_bin_day_council_found(self, mock_driver, mock_file):
        ...
        with patch("main.Select") as mock_select:
            mock_select.return_value.options = [MagicMock(text="1 High Street")]
            ...
            with patch("builtins.input", return_value="1 High Street"):
                with patch("builtins.print") as mock_print:
                    ...
                    mock_driver.find_element.side_effect = [MagicMock(), mock_elem, MagicMock(), mock_elem]
    ...
    def test_next_bin_day_council_not_found(self, mock_driver, mock_file):
        with patch("builtins.print") as mock_print:
            ...
            mock_print.assert_any_call("Bin collection information for CouncilY could not be found.")
